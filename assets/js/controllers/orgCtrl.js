...
window.globalBUs = window.globalBUs || [];
window.globalTeams = window.globalTeams || [];
window.globalTerritories = window.globalTerritories || [];
window.globalProducts = window.globalProducts || [];
window.globalProductTeams = window.globalProductTeams || [];
window.currentHubTeamId = window.currentHubTeamId || "";

// 🛠️ Helper: Get Current User Email
window.getOrgUserEmail = function() {
  try {
...
window.forceReloadOrgData = async function () {
  try {
    ...
    window.globalBUs = buRes.data || [];
    window.globalTeams = teamRes.data || [];
    window.globalTerritories = terrRes.data || [];
    window.globalProducts = prodRes.data || [];
    window.globalProductTeams = ptRes.data || [];

    if (typeof window.renderTeamsHub === 'function') window.renderTeamsHub();
    if (typeof window.renderMasterTables === 'function') window.renderMasterTables();
    if (typeof window.populateOrgDropdowns === 'function') window.populateOrgDropdowns();

  } catch (err) {
    console.error("❌ Error loading Org Data:", err.message);
    const tbody = document.getElementById('hubTeamTableBody');
    if (tbody) tbody.innerHTML = `<tr><td colspan="5" class="py-4 text-danger">❌ Error loading data: ${err.message}</td></tr>`;
  }
};
...
window.switchOrgSubView = function (targetViewId, module) {
  ...
  if (targetViewId.includes('List')) {
    if (module === 'team' && document.getElementById('addTeamName')) document.getElementById('addTeamName').value = '';
    if (module === 'product' && document.getElementById('addProductName')) document.getElementById('addProductName').value = '';
    if (module === 'territory' && document.getElementById('addTerritoryName')) document.getElementById('addTerritoryName').value = '';
    if (module === 'bu' && document.getElementById('addBuName')) document.getElementById('addBuName').value = '';
  }
};

// 🌟 Dropdown Populator
window.populateOrgDropdowns = function() {
  let buOptions = '<option value="">- Select BU -</option>';
  (window.globalBUs || []).filter(b => b.Status === 'Active').forEach(b => {
  ...
  (window.globalTeams || []).filter(t => t.Status === 'Active').forEach(t => {
...
// ==================== 1. TEAMS HUB LOGIC ====================
window.renderTeamsHub = function() {
  const tbody = document.getElementById('hubTeamTableBody');
  if (!tbody) return;
  tbody.innerHTML = '';
  ...
  window.globalTeams.forEach(t => {
    const buObj = (window.globalBUs || []).find(b => b.BU_ID === t.BU_ID);
    const buName = buObj ? buObj.BU : '-';
    const terrCount = (window.globalTerritories || []).filter(tr => tr.Team_ID === t.Team_ID).length;
    const prodCount = (window.globalProductTeams || []).filter(pt => pt.Team_ID === t.Team_ID).length;
...
window.handleAddTeam = async function (e) {
  ...
  const payload = {
    ...
    Status: 'Active',
    Whoupdated: window.getOrgUserEmail()
  };
  try {
    const sb = window.supabaseClient || window.supabase;
...
window.openTeamHub = function (teamId) {
  window.currentHubTeamId = teamId;
  const t = (window.globalTeams || []).find(x => x.Team_ID === teamId);
  if (!t) return;

  if (document.getElementById('hubTeamName')) document.getElementById('hubTeamName').innerText = t.Team;
  ...
  if (document.getElementById('hubBuSelect')) document.getElementById('hubBuSelect').innerHTML = buHtml;

  // Team Products
  const myProdTeams = (window.globalProductTeams || []).filter(pt => pt.Team_ID === teamId);
  let prodHtml = '';
  myProdTeams.forEach(pt => {
    const pInfo = (window.globalProducts || []).find(p => p.Product_ID === pt.Product_ID);
    if (pInfo) {
      prodHtml += `
  ...
  if (document.getElementById('hubProductList')) {
  ...
  // Team Territories
  const myTerrs = (window.globalTerritories || []).filter(tr => tr.Team_ID === teamId);
  let terrHtml = '';
  myTerrs.forEach(tr => {
    terrHtml += `
      <li class="list-group-item px-0 d-flex justify-content-between align-items-center border-0 py-1">
  ...
  if (document.getElementById('hubTerritoryList')) {
  ...
  (window.globalProducts || []).filter(p => p.Status === 'Active' && !myProdTeams.some(pt => pt.Product_ID === p.Product_ID)).forEach(p => {
    addProdHtml += `<option value="${p.Product_ID}">${p.Product}</option>`;
  });
  if (document.getElementById('hubAddProductSelect')) document.getElementById('hubAddProductSelect').innerHTML = addProdHtml;

  // Add Territory Dropdown
  let addTerrHtml = '<option value="">- Add Territory -</option>';
  (window.globalTerritories || []).filter(tr => tr.Status === 'Active' && tr.Team_ID !== teamId).forEach(tr => {
    addTerrHtml += `<option value="${tr.Territory_ID}">${tr.Territory}</option>`;
  });
  if (document.getElementById('hubAddTerritorySelect')) document.getElementById('hubAddTerritorySelect').innerHTML = addTerrHtml;
...
window.updateTeamBU = async function () {
  ...
  try {
    const sb = window.supabaseClient || window.supabase;
    const { error } = await sb.from('Team').update({ BU_ID: newBuId, Whoupdated: window.getOrgUserEmail() }).eq('Team_ID', window.currentHubTeamId);
    if (error) throw error;
...
window.addProductToTeam = async function () {
  ...
  try {
    const sb = window.supabaseClient || window.supabase;
    const { error } = await sb.from('Products_Team').insert([{ Product_ID: pId, Team_ID: window.currentHubTeamId, Status: 'Active', Whoupdated: window.getOrgUserEmail() }]);
    if (error) throw error;

    await window.forceReloadOrgData();
    window.openTeamHub(window.currentHubTeamId);
  } catch (err) {
...
window.removeProductFromTeam = async function (productId) {
  ...
  try {
    const sb = window.supabaseClient || window.supabase;
    await sb.from('Products_Team').delete().eq('Product_ID', productId).eq('Team_ID', window.currentHubTeamId);
    await window.forceReloadOrgData();
    window.openTeamHub(window.currentHubTeamId);
  } catch (err) {
...
window.addTerritoryToTeam = async function () {
  ...
  try {
    const sb = window.supabaseClient || window.supabase;
    const { error } = await sb.from('Territory').update({ Team_ID: window.currentHubTeamId, Whoupdated: window.getOrgUserEmail() }).eq('Territory_ID', tId);
    if (error) throw error;
...
window.renderMasterTables = function() {
  ...
  // 2.2 Territory
  let tBody = '';
  (window.globalTerritories || []).forEach(t => {
    const badge = t.Status === 'Active' ? 'badge-soft-success' : 'badge-soft-secondary';
    const teamObj = (window.globalTeams || []).find(tm => tm.Team_ID === t.Team_ID);
...
window.handleAddProduct = async function (e) {
  ...
  try {
    const sb = window.supabaseClient || window.supabase;
    await sb.from('Products').insert([{ Product: document.getElementById('addProductName').value.trim(), Status: 'Active', Whoupdated: window.getOrgUserEmail() }]);
...
window.openEditProduct = function (id) {
  const p = (window.globalProducts || []).find(x => x.Product_ID === id);
  if (p) {
...
window.handleEditProduct = async function (e) {
  ...
  try {
    const sb = window.supabaseClient || window.supabase;
    await sb.from('Products').update({ Product: document.getElementById('editProductName').value.trim(), Status: document.getElementById('editProductStatus').value, Whoupdated: window.getOrgUserEmail() }).eq('Product_ID', document.getElementById('editProductId').value);
    window.switchOrgSubView('productListView', 'product');
    await window.forceReloadOrgData();
  } catch (err) { alert("❌ Error: " + err.message); }
};
...
window.handleAddTerritory = async function (e) {
  ...
  try {
    const sb = window.supabaseClient || window.supabase;
    await sb.from('Territory').insert([{ Territory: document.getElementById('addTerritoryName').value.trim(), Team_ID: tId || null, Status: 'Active', Whoupdated: window.getOrgUserEmail() }]);
...
window.openEditTerritory = function (id) {
  const t = (window.globalTerritories || []).find(x => x.Territory_ID === id);
  if (t) {
...
window.handleEditTerritory = async function (e) {
  ...
  try {
    const sb = window.supabaseClient || window.supabase;
    await sb.from('Territory').update({ Territory: document.getElementById('editTerritoryName').value.trim(), Team_ID: tId || null, Status: document.getElementById('editTerritoryStatus').value, Whoupdated: window.getOrgUserEmail() }).eq('Territory_ID', document.getElementById('editTerritoryId').value);
    window.switchOrgSubView('territoryListView', 'territory');
    await window.forceReloadOrgData();
  } catch (err) { alert("❌ Error: " + err.message); }
};
...
window.handleAddBU = async function (e) {
  ...
  try {
    const sb = window.supabaseClient || window.supabase;
    await sb.from('BU').insert([{ BU: document.getElementById('addBuName').value.trim(), Status: 'Active', Whoupdated: window.getOrgUserEmail() }]);
...
window.openEditBU = function (id) {
  const b = (window.globalBUs || []).find(x => x.BU_ID === id);
  if (b) {
...
window.handleEditBU = async function (e) {
  ...
  try {
    const sb = window.supabaseClient || window.supabase;
    await sb.from('BU').update({ BU: document.getElementById('editBuName').value.trim(), Status: document.getElementById('editBuStatus').value, Whoupdated: window.getOrgUserEmail() }).eq('BU_ID', document.getElementById('editBuId').value);
    window.switchOrgSubView('buListView', 'bu');
    await window.forceReloadOrgData();
  } catch (err) { alert("❌ Error: " + err.message); }
};

// Auto Execute Direct Load
if (document.readyState === 'complete' || document.readyState === 'interactive') {
  setTimeout(window.forceReloadOrgData, 100);
} else {
