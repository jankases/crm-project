      ...
      if (typeof TomSelect !== 'undefined') {
        ...
        // 5.2 🌟 เปลี่ยน Purpose ให้เป็น Checkbox Tree เหมือน Employee/Area
          if (filterPurposeSelect) {
              ...
              // สร้างกล่อง Container สไตล์เดียวกับ Employee
              var containerId = 'containerFilterPurpose';
              ...
              // แปลงข้อมูลให้อยู่ในรูปแบบ Tree 
              var purposeTree = [{
                  id: 'fld_purpose_all',
                  text: '🎯 ' + (appLang === 'th' ? 'วัตถุประสงค์ทั้งหมด' : 'All Purposes'),
                  isLeaf: false,
                  children: purposeData.map(function(p) {
                      return { id: 'purp_' + p.value, value: p.value, text: p.text, isLeaf: true };
                  })
              }];

              // สั่งวาด Checkbox ลงกล่อง
              window.renderTreeCheckboxList(containerId, purposeTree, 'purpose');
          }
      }
    }
...
window.setupFiltersDropdowns = async function(crmUser, productsTeamList) {
    ...
    try {
        ...
        var allTers = window.globalTerritoryList || [];
        if (allTers.length === 0) {
            try { var { data: trData } = await sb.from('Territory').select('*'); allTers = trData || []; window.globalTerritoryList = allTers; } catch(e){}
        }
        var allTms = window.globalTeamList || [];
        if (allTms.length === 0) {
            try { var { data: tmData } = await sb.from('Team').select('*'); allTms = tmData || []; window.globalTeamList = allTms; } catch(e){}
        }
        var allBus = window.globalBuList || [];
        if (allBus.length === 0) {
            try { var { data: buData } = await sb.from('BU').select('*'); allBus = buData || []; window.globalBuList = allBus; } catch(e){}
        }
        var usersSource = window.globalUsersList || [];
        if (usersSource.length === 0) {
            try { var { data: allUsers } = await sb.from('Rep_Users').select('*'); usersSource = allUsers || []; window.globalUsersList = usersSource; } catch(e){}
        }
        ...
        if (uRepId && usersSource.length > 0) {
            var dbMe = usersSource.find(u => String(u.Rep_ID || u.id || '').trim().toLowerCase() === uRepId.toLowerCase());
            if (dbMe) {
                if (!userTeamId) userTeamId = String(dbMe.Team_ID || dbMe.Team || '').trim().toLowerCase();
                if (!userTerrId) userTerrId = String(dbMe.Territory_ID || dbMe.Territory || '').trim().toLowerCase();
                if (!userBuId) userBuId = String(dbMe.BU_ID || dbMe.BU || '').trim().toLowerCase();
            }
        }
        ...
        if (!isGlobalViewer) {
            if (isBuHead && userBuId) {
                var buTeams = allTms.filter(t => String(t.BU_ID || t.BU || '').trim().toLowerCase() === userBuId);
                buTeams.forEach(t => myAllowedTeamIds.push(String(t.Team_ID || t.id).toLowerCase()));

                if (myAllowedTeamIds.length > 0) {
                    var buTers = allTers.filter(t => myAllowedTeamIds.includes(String(t.Team_ID || t.Team || '').trim().toLowerCase()));
                    buTers.forEach(t => myAllowedTerIds.push(String(t.Territory_ID || t.id).toLowerCase()));
                }
            } 
            else if (isManager && userTeamId) {
                myAllowedTeamIds.push(userTeamId);
                var mgrTers = allTers.filter(t => String(t.Team_ID || t.Team || '').trim().toLowerCase() === userTeamId);
                mgrTers.forEach(t => myAllowedTerIds.push(String(t.Territory_ID || t.id).toLowerCase()));
            } 
            ...
            usersSource.forEach(function(u) {
                ...
                var isAdminRole = uRoleStr.indexOf('ADMIN') !== -1 || uRoleStr.indexOf('STAFF') !== -1 || uRoleStr.indexOf('DIRECTOR') !== -1 || uRoleStr.indexOf('EXECUTIVE') !== -1 || uRoleStr.indexOf('SECRETARY') !== -1;
                ...
                if (!isAdminRole) {
                    ...
                    if (!isSales && !isProductManager) {
                        ...
                        if (isMatchTeam || isMatchTer || uid === uRepId.toLowerCase() || myAllowedRepIds.indexOf(uid) !== -1) {
                            if (uid && myAllowedRepIds.indexOf(uid) === -1) myAllowedRepIds.push(uid);
                        }
                    }
                }
        ...
        var allowedRepArrayForDropdown = isGlobalViewer 
            ? usersSource 
            : usersSource.filter(function(u) {
                var uid = String(u.Rep_ID || u.User_ID || u.id || '').trim().toLowerCase(); 
                return myAllowedRepIds.indexOf(uid) !== -1;
            });

        // ==============================================
        // 🎯 ปั้นโครงสร้าง AREA / TEAM (Role-Based Depth)
        // ==============================================
        var terOptionsTree = [];
        var buMapTer = {}; 
        var tmMapTer = {}; 
        ...
        allowedTerArrayForDropdown.forEach(function(t) {
            ...
            if (tid) {
                ...
                if (isSales) {
                    terOptionsTree.push(nodeTer); // Sales: โชว์แค่เขต ไม่มีโฟลเดอร์
                } 
                else if (isManager) {
                    if (!tmMapTer[teamId]) {
                        var teamName = tmObj ? (tmObj.Team || tmObj.Team_Name || teamId) : '';
                        if (!teamName) teamName = (teamId === 'no_team') ? 'Territories' : (teamId.length > 15 ? 'Sales Team' : teamId);
                        tmMapTer[teamId] = { id: 'tm_ter_' + teamId, text: '👥 ' + teamName, isLeaf: false, children: [] };
                        
                        // 🎯 [FIX]: คืนชีพโหนด (Team Level) ให้ Manager ไว้ติ๊กดูผลงานของทีม
                        if (teamId !== 'no_team') {
                            var valTeam = tmObj ? (tmObj.Team_ID || tmObj.id || teamId) : teamId;
                            tmMapTer[teamId].children.push({ id: 'ter_tm_' + teamId, value: valTeam, text: '👥 ' + teamName + ' (Team Level)', isLeaf: true });
                        }
                    }
                    tmMapTer[teamId].children.push(nodeTer);
                } 
                else {
                    var rootBuId = (isBuHead && userBuId) ? userBuId : buId; 
                    var buObj = allBus.find(function(b) { return String(b.BU_ID || b.id || b.BU || '').trim().toLowerCase() === rootBuId; });
                    var buName = buObj ? (buObj.BU_Name || buObj.BU) : '';
                    
                    if (!buName) {
                        if (rootBuId === 'no_bu' || rootBuId === 'other_bu') buName = 'Other / Cross-Area';
                        else if (rootBuId.length > 15) buName = 'Business Unit'; // ซ่อน UUID
                        else buName = rootBuId;
                    }
                    ...
                    if (!buMapTer[rootBuId]) {
                        ...
                        // 🎯 [FIX]: คืนชีพโหนด (BU Level) ให้ BU Head / Admin ไว้ติ๊กดูผลงานของ BU ตัวเอง
                        if (rootBuId !== 'no_bu' && rootBuId !== 'other_bu') {
                            var valBu = buObj ? (buObj.BU_ID || buObj.id || rootBuId) : rootBuId;
                            buMapTer[rootBuId].children.push({ id: 'ter_bu_' + rootBuId, value: valBu, text: icon + buName + ' (BU Level)', isLeaf: true });
                        }
                    }
                    
                    var currentBuNode = buMapTer[rootBuId];
                    if (!currentBuNode._teamMap[teamId]) {
                        var teamName2 = tmObj ? (tmObj.Team || tmObj.Team_Name || teamId) : '';
                        if (!teamName2) teamName2 = (teamId === 'no_team') ? 'Unassigned Team' : (teamId.length > 15 ? 'Sales Team' : teamId);
                        
                        var newTeamNode = { id: 'tm_ter_' + teamId, text: '👥 ' + teamName2, isLeaf: false, children: [] };
                        
                        if (teamId !== 'no_team' && !isSales) { 
                            var valTeam2 = tmObj ? (tmObj.Team_ID || tmObj.id || teamId) : teamId;
                            newTeamNode.children.push({ id: 'ter_tm_' + teamId, value: valTeam2, text: '👥 ' + teamName2 + ' (Team Level)', isLeaf: true });
                        }
        ...
        // ==============================================
        // 🎯 ปั้นข้อมูล EMPLOYEE / SALES REP (Role-Based Depth)
        // ==============================================
        var repOptionsTree = [];
        var buMapRep = {}; 
        var tmMapRep = {}; 
        ...
        allowedRepArrayForDropdown.forEach(function(u) {
            ...
            if (id && id !== 'undefined' && id !== 'null' && !uniqueUsersMap.has(id.toLowerCase())) {
                ...
                if (isSales) {
                    repOptionsTree.push(nodeRep); // Sales: โชว์แค่ชื่อ ไม่มีโฟลเดอร์
                } 
                else if (isManager) {
                    if (!tmMapRep[teamId]) {
                        var tmObjRep = allTms.find(function(tm) { return String(tm.Team_ID || tm.id || tm.Team || '').trim().toLowerCase() === teamId; });
                        var teamNameRep = tmObjRep ? (tmObjRep.Team || tmObjRep.Team_Name || teamId) : '';
                        if (!teamNameRep) teamNameRep = (teamId === 'no_team') ? 'Unassigned Team' : (teamId.length > 15 ? 'Sales Team' : teamId);
                    ...
                    var rBuName = rBuObj ? (rBuObj.BU_Name || rBuObj.BU) : '';
                    
                    if (!rBuName) {
                        if (rootBuId === 'no_bu' || rootBuId === 'other_bu') rBuName = 'Other / Cross-Team';
                        else if (rootBuId.length > 15) rBuName = 'Business Unit'; // ซ่อน UUID
                        else rBuName = rootBuId;
                    }
                    var icon2 = (rootBuId === 'no_bu' || rootBuId === 'other_bu') ? '🌐 ' : '🏢 ';

                    if (!buMapRep[rootBuId]) {
                        buMapRep[rootBuId] = {
                            id: 'bu_rep_' + rootBuId, text: icon2 + rBuName, isLeaf: false, children: [],
                    ...
                    if (isAdminOrStaff) currentBuRepNode._staffFolder.children.push(nodeRep);
                        ...
                        if (!currentBuRepNode._teams[teamId]) {
                            ...
                            var teamNameRep2 = tmObjRep2 ? (tmObjRep2.Team || tmObjRep2.Team_Name || teamId) : '';
                            if (!teamNameRep2) teamNameRep2 = (teamId === 'no_team') ? 'Unassigned Team' : (teamId.length > 15 ? 'Sales Team' : teamId);
        ...
        // ==============================================
        // 🎯 วาดรายการ Tree-View Checkbox เข้าใน HTML
        // ==============================================
...
window.loadVisits = async function(forceReload, isBackground) {
    // 🎯 [Fix 1]: ฝังระบบ Request ID ป้องกันข้อมูลวิ่งแซงกัน
    window._visitQueryId = (window._visitQueryId || 0) + 1;
    var currentQueryId = window._visitQueryId;

    // 🚨🚨 THE MASTER FIX: งับค่าจากหน้าจอ "ทันที" ตั้งแต่วินาทีแรก! 🚨🚨
    // (ก่อนที่หน้าต่าง Filter จะถูกปิด และก่อนที่ระบบจะไป await คุยกับ Database)
    var statusEl = document.getElementById('filterVisitStatus');
    var capturedStatus = statusEl ? statusEl.value : '';
    ...
    document.querySelectorAll('.chk-tree-rep.chk-leaf:checked').forEach(function(c) {
        if (c.value && c.value !== 'on') capturedReps.push(c.value.trim());
    });
    if (document.querySelector('input[id*="other_reps"]:checked') && capturedReps.length === 0) capturedReps.push('EMPTY_OTHER');

    var capturedTers = [];
    document.querySelectorAll('.chk-tree-ter.chk-leaf:checked').forEach(function(c) {
        if (c.value && c.value !== 'on') capturedTers.push(c.value.trim());
    });
    if (document.querySelector('input[id*="other_ter"]:checked') && capturedTers.length === 0) capturedTers.push('EMPTY_OTHER');
    // ==============================================================
    ...
    try {
      ...
      if (!isGlobalAdmin) {
          if (isProductManager) {
              ...
              // ดึง Territory ทั้งหมดในทีมของ BU
              var ownTerIds = [];
              if (ownTeamIds.length > 0) {
                  var { data: buTers } = await sb.from('Territory').select('Territory_ID').in('Team_ID', ownTeamIds);
                  ownTerIds = (buTers || []).map(function(t) { return String(t.Territory_ID).toLowerCase(); });
              }
              ...
              // 3. รวบรวม Rep_ID (ลูกทีมตัวเองแบบเจาะลึกทุกระดับชั้น)
              var allowedReps = [];
              (window.globalUsersList || []).forEach(function(u) {
                  var uid = String(u.Rep_ID || u.User_ID || u.id || '').trim().toLowerCase();
                  var uteam = String(u.Team_ID || u.Team || '').trim().toLowerCase();
                  ...
                  if (!isAdminRole) {
                      var isMatchTeam = ownTeamIds.indexOf(uteam) !== -1;
                      var isMatchTer = ownTerIds.indexOf(uter) !== -1;
                      var isMatchBu = uBu === String(window.myUserBuId).toLowerCase();
                      
                      // ถ้าอยู่ใต้ BU นี้ในทางใดทางหนึ่ง ให้รวบชื่อมาให้หมด!
                      if (isMatchTeam || isMatchTer || isMatchBu || uid === String(myRepId).toLowerCase()) {
                          if (uid && allowedReps.indexOf(uid) === -1) allowedReps.push(uid);
                      }
                  }
              });

              // ดึง PM หรือคนนอกทีม ที่มี Product ตรงกับ BU (จากตาราง Rep_Products)
              if (buProdIds.length > 0) {
                  ...
                  if (crossReps) {
                      crossReps.forEach(function(rp) {
                          ...
                          if (rId && allowedReps.indexOf(rId) === -1) allowedReps.push(rId);
                      });
                  }
              }
              ...
              // 5. จับรวมกัน (เป็นลูกทีมตัวเอง OR เป็น Visit ที่มีสินค้าตัวเอง)
              var orConditions = [];
              if (allowedReps.length > 0) {
                  // 🚨 แก้บั๊ก: ลบเครื่องหมาย " ออกจาก UUID 
                  var cleanRepIds = allowedReps.join(',');
              ...
              if (buVisitIds.length > 0) {
                  ...
                  // 🚨 แก้บั๊ก: ลบเครื่องหมาย " ออกจาก UUID
                  var cleanVisitIds = safeBuVisitIds.join(',');
     ...
     // ==============================================================
      ...
      if (capturedStartDate) {
          dataQuery = dataQuery.gte('Visit_Date', capturedStartDate);
          countQuery = countQuery.gte('Visit_Date', capturedStartDate);
      }

      if (capturedEndDate) {
          dataQuery = dataQuery.lte('Visit_Date', capturedEndDate);
          countQuery = countQuery.lte('Visit_Date', capturedEndDate);
      }

      if (capturedReps.length > 0) {
          if (capturedReps.indexOf('EMPTY_OTHER') !== -1) {
              dataQuery = dataQuery.is('Rep_ID', null);
              countQuery = countQuery.is('Rep_ID', null);
          } else {
              dataQuery = dataQuery.in('Rep_ID', capturedReps);
              countQuery = countQuery.in('Rep_ID', capturedReps);
          }
      }

      if (capturedTers.length > 0) {
          if (capturedTers.indexOf('EMPTY_OTHER') !== -1) {
              dataQuery = dataQuery.is('Territory_ID', null);
              countQuery = countQuery.is('Territory_ID', null);
          } else {
              dataQuery = dataQuery.in('Territory_ID', capturedTers);
              countQuery = countQuery.in('Territory_ID', capturedTers);
          }
      }
 ...
 window.renderFormProductDropdown = async function() {
    ...
    var allProds = (window.globalProductsList && window.globalProductsList.length > 0) 
        ...
        : (window.VisitManagerCache ? window.VisitManagerCache.products : []);
...
window.restoreVisitFilterState = function() {
    ...
    try {
        ...
        document.querySelectorAll('input[type="checkbox"].chk-tree-rep, input[type="checkbox"].chk-tree-ter, input[type="checkbox"].chk-tree-purpose').forEach(function(c) { 
            c.checked = false; 
        ...
        // 🌟 คืนชีพ Checkbox 
        var types = [
            { className: 'input[type="checkbox"].chk-tree-purpose', data: f.purpose, typeStr: 'purpose' },
            { className: 'input[type="checkbox"].chk-tree-rep', data: f.reps, typeStr: 'rep' },
            { className: 'input[type="checkbox"].chk-tree-ter', data: f.ters, typeStr: 'ter' }
        ];
...
window.updateCalendarLegendLang = function() {
    ...
    // 🌟 สั่งวาด Dropdown พนักงานใน Calendar ใหม่ทั้งก้อน เพื่อให้แปลภาษาครบทุกโฟลเดอร์
    if (typeof window.renderCalendarUserDropdown === 'function') {
        window.renderCalendarUserDropdown('calRepFilterSelect');
    }
    ...
    try {
        ...
        if (data && data.length > 0) {
            ...
            if (lastVisit.Visit_Date) {
                ...
                // 🌟 [FIX 1] ปรับ Format วันที่ให้เป็น DD/MM/YYYY ตามมาตรฐานฟอร์มหลัก
                formattedDate = (typeof formatToDDMMYYYY === 'function') ? formatToDDMMYYYY(lastVisit.Visit_Date) : lastVisit.Visit_Date.split('T')[0];

                const today = new Date();
                today.setHours(0,0,0,0);
                const lastDateCompare = new Date(lastDateObj);
                lastDateCompare.setHours(0,0,0,0);
                
                const diffTime = today - lastDateCompare;
                const diffDays = Math.round(diffTime / (1000 * 60 * 60 * 24));
                
                // 🌟 [FIX 2] แก้ไข Grammar (Singular/Plural) ให้ถูกต้องเป๊ะๆ
                if (diffDays >= 0) {
                    if (diffDays === 0) {
                        relativeTimeStr = appLang === 'en' ? 'Today' : 'วันนี้';
                    } else if (diffDays === 1) {
                        relativeTimeStr = appLang === 'en' ? 'Yesterday' : 'เมื่อวาน';
                    } else if (diffDays < 7) {
                        relativeTimeStr = appLang === 'en' ? `${diffDays} days ago` : `${diffDays} วันที่แล้ว`;
                    } else if (diffDays < 30) {
                        var w = Math.floor(diffDays / 7);
                        relativeTimeStr = appLang === 'en' ? `${w} week${w > 1 ? 's' : ''} ago` : `${w} สัปดาห์ที่แล้ว`;
                    } else if (diffDays < 365) {
                        var m = Math.floor(diffDays / 30);
                        relativeTimeStr = appLang === 'en' ? `${m} month${m > 1 ? 's' : ''} ago` : `${m} เดือนที่แล้ว`;
                    } else {
                        var y = Math.floor(diffDays / 365);
                        relativeTimeStr = appLang === 'en' ? `${y} year${y > 1 ? 's' : ''} ago` : `${y} ปีที่แล้ว`;
                    }
                }
            }
            ...
            if (dateBadge) {
                ...
                // 🌟 [Premium UI] ปรับป้ายให้ขอบมนแบบแคปซูล และใช้สี Soft (Subtle) เพื่อความหรูหรา
                if (lastVisitDateObj > currentSelectedDate) {
                    dateBadge.className = 'badge bg-warning-subtle text-warning-emphasis border border-warning-subtle shadow-sm px-3 py-2 rounded-pill';
                    var warnText = appLang === 'en' ? 'Backdated' : 'บันทึกย้อนหลัง';
                    dateBadge.innerHTML = `<i class="fa-solid fa-clock-rotate-left me-1"></i> ${displayText} <span class="ms-1 text-danger fw-bold">[${warnText}]</span>`;
                } else {
                    dateBadge.className = 'badge bg-primary-subtle text-primary-emphasis border border-primary-subtle shadow-sm px-3 py-2 rounded-pill';
                    dateBadge.innerHTML = `<i class="fa-solid fa-calendar-check me-1"></i> ${displayText}`;
                }
            }
 ...
 window.renderCoachDropdown = function() {
    ...
    if (window.globalUsersList && window.globalUsersList.length > 0) {
        window.globalUsersList.forEach(function(u) {
            ...
            var uTerrId = String(u.Territory_ID || u.Territory || '').trim().toLowerCase(); // <-- ดึง Territory ของคนในลิสต์มาด้วย

            // 🎯 [Fix 3]: ถ้า Manager/รายชื่อคนอื่น ไม่มี Team ให้ไปแอบดูจาก Territory แทน
            if (!uTeamId && uTerrId) {
                var uTerrList = window.globalTerritoryList || (window.VisitManagerCache ? window.VisitManagerCache.territories : []) || [];
                var uMatchedTer = uTerrList.find(function(t) { return String(t.Territory_ID || t.id || t.Territory || '').trim().toLowerCase() === uTerrId; });
                if (uMatchedTer) {
                    uTeamId = String(uMatchedTer.Team_ID || uMatchedTer.Team || '').trim().toLowerCase();
                }
            }
...
window.renderCalendarUserDropdown = function(selectElementId) {
    ...
    // 🌟 จำค่าเดิมที่เลือกไว้ จะได้ไม่หลุดเวลาเปลี่ยนภาษา
    var currentSelected = window.currentCalendarRepFilter || 'ALL';
    ...
    var crmUser = null; 
    try { crmUser = JSON.parse(sessionStorage.getItem('crmUser')); } catch(e){}
    var myRepId = crmUser ? String(crmUser.Rep_ID || crmUser.id || '').trim().toLowerCase() : '';
    var myBuId = crmUser ? String(crmUser.BU_ID || crmUser.BU || '').trim().toLowerCase() : '';
    ...
    allowedUsers.forEach(function(u) {
        ...
        if (!uBuId && uid === myRepId) uBuId = myBuId;
        ...
        if (isAdminOrStaff) {
            groupKey = 'admin';
        } else if (isNodeBuHead || isNodePM) {
            groupKey = uBuId ? 'bu_' + uBuId : 'other';
        } else if (teamId && teamId !== 'no_team') {
    ...
    // 🌟 แปลงภาษาข้อความ All Users
    var allUsersText = isEN ? '👥 All Users' : '👥 พนักงานทั้งหมด';
    var isAllSel = (currentSelected === 'ALL' || !currentSelected) ? 'selected' : '';
    var html = '<option value="ALL" ' + isAllSel + '>' + allUsersText + '</option>';
    
    Object.keys(groupedUsers).forEach(function(key) {
        // 🌟 แปลงภาษาชื่อโฟลเดอร์ต่างๆ
        var groupName = isEN ? 'Unassigned Team' : 'ทีมที่ไม่ได้ระบุ';
        
        if (key.startsWith('team_')) {
            var tId = key.replace('team_', '');
            var tmObj = allTms.find(function(tm) { return String(tm.Team_ID || tm.id || tm.Team).toLowerCase() === tId; });
            groupName = tmObj ? (tmObj.Team || tmObj.Team_Name || tId) : (tId.length > 15 ? (isEN ? 'Sales Team' : 'ทีมขาย') : tId);
            groupName = '👥 ' + groupName;
            ...
            groupName = buObj ? (buObj.BU_Name || buObj.BU) : bId;
            if (!groupName || groupName.length > 15) groupName = isEN ? 'Business Unit' : 'หน่วยธุรกิจ';
            groupName = '🏢 ' + groupName + (isEN ? ' (BU Level)' : ' (ระดับ BU)'); 
        } else if (key === 'admin') {
            groupName = isEN ? '🛡️ Admin / Global' : '🛡️ แอดมิน / ส่วนกลาง';
        } else {
            groupName = isEN ? '🌐 Other / Cross-Team' : '🌐 บุคคลอื่น / นอกทีม';
        }
        ...
        sortedUsers.forEach(function(u) {
            ...
            // 🌟 ติ๊กเลือกค่าเดิมที่ค้างไว้
            var isSel = (id === currentSelected) ? 'selected' : '';
            html += '<option value="' + id + '" ' + isSel + '>' + nodeIcon + name + '</option>';
        });
...
// ==========================================
// 📄 PAGINATION & TABLE RENDER ENGINE (ULTIMATE FIX)
// ==========================================

// 🎯 1. ฟังก์ชันเปลี่ยนจำนวนแถว (ควานหาค่าเอง ทะลวง ID ซ้ำซ้อน)
window.changeRowsPerPage = function(el) {
    var newRows = 20;
    
    // 1. ลองดึงจาก Element ที่คลิกโดยตรง
    if (el && el.value) {
        newRows = parseInt(el.value, 10);
    } 
    // 2. ลองดึงจาก Event ปัจจุบัน
    else if (window.event && window.event.target && window.event.target.value) {
        newRows = parseInt(window.event.target.value, 10);
    } 
    // 3. ไม้ตาย: ควานหา Select ทุกกล่องบนหน้าจอ ถ้ากล่องไหนเลขเปลี่ยนไปจากเดิม ให้ยึดค่านั้น!
    else {
        var allSelects = document.querySelectorAll('select[onchange*="changeRowsPerPage"]');
        for (var i = 0; i < allSelects.length; i++) {
            var v = parseInt(allSelects[i].value, 10);
            if (v && v !== window.rowsPerPage) {
                newRows = v;
                break;
            }
        }
    ...
    // อัปเดตค่าให้ระบบ
    window.rowsPerPage = newRows || 20;

    // บังคับซิงค์กลับให้ Select ทุกกล่องมีเลขตรงกัน
    document.querySelectorAll('select[onchange*="changeRowsPerPage"]').forEach(function(sel) {
        sel.value = window.rowsPerPage;
    });
...
window.goToPage = function(page, event) {
    // ดักจับ Event ทันทีถ้ามีการส่งมา
...
window.renderVisitTableServerSide = function() {
    ...
    document.querySelectorAll('select[onchange*="changeRowsPerPage"]').forEach(function(sel) {
        if (sel.value != window.rowsPerPage) sel.value = window.rowsPerPage || 20;
    });
    ...
    pageData.forEach(function(v) {
        ...
        var evidenceBadges = '';
        if (v.Is_Coaching) {
          evidenceBadges += ' <span class="badge badge-soft-info ms-1"><i class="fa-solid fa-clipboard-user text-info"></i></span>';
        }

        if (window.globalVisitConfigs && window.globalVisitConfigs.att !== false && v.Attachments && v.Attachments !== '[]' && v.Attachments !== '') {
          evidenceBadges += ' <span class="badge badge-soft-secondary ms-1"><i class="fa-solid fa-paperclip text-secondary"></i></span>';
        }

        if (window.globalVisitConfigs && window.globalVisitConfigs.sig !== false && v.Doctor_Signature) {
          evidenceBadges += ' <span class="badge badge-soft-success ms-1"><i class="fa-solid fa-signature text-success"></i></span>';
        }
...
// 🛑 ไม้ตายสุดท้าย: สกัดกั้น URL # ด้วย Capture Phase (ดักทางก่อนที่ Browser จะขยับตัว)
if (!window._urlHashKillerAttached) {
