...
window.loadDropdowns = async function(forceReload) {
  ...
  try {
    ...
    // ✅ คืนชีพ: ส่วนคืนค่าหมอเมื่อเด้งกลับมาจากหน้า Doctor Profile
...
window.setupFiltersDropdowns = function(crmUser, productsTeamList) {
    ...
    // 🚨 ลบเงื่อนไข "if (!repSelect || !terSelect) return;" ทิ้งไปเลย!
    // เราต้องให้มันประมวลผลสิทธิ์ (myAllowedRepIds) เสมอ แม้ว่ากล่อง HTML จะยังไม่พร้อมก็ตาม
    ...
    // 🌟 พระเอกอยู่ตรงนี้: ต่อให้กล่อง HTML ยังไม่พร้อม ตัวแปรเก็บสิทธิ์เหล่านี้ก็ต้องถูกสร้างขึ้นมาเสมอ!
    ...
    // 🛡️ ป้องกันบั๊ก HTML: เอาโค้ดที่อัปเดตหน้าจอมาใส่ไว้ในเงื่อนไข If แทน
    if (repSelect) {
...
// 🌟 1. ดึงตำแหน่ง (Role) ของ User ออกมาเช็กด้วย
    var myRole = crmUser ? String(crmUser.Role || crmUser.role || '').trim().toLowerCase() : '';
    var isGlobalAdmin = window.myIsGlobalViewer; // ใช้ค่าสถานะที่ซิงค์มาจาก loadDropdowns แน่นอนที่สุด
    ...
    if (rawSearchVal) {
        // แยกคำค้นหาด้วยเว้นวรรค เช่น "รามา t" -> ["รามา", "t"]
        ...
        for (var i = 0; i < searchTerms.length; i++) {
            ...
            // 4. เอามาประกอบร่างเป็นเงื่อนไขส่งให้ฐานข้อมูลทีละคำ
            if (matchedDocIds.length > 0 || matchedVisitIds.length > 0) {
                var safeDocIds = matchedDocIds.slice(0, 60); // ตัด 60 รายการ ป้องกัน Failed to fetch
                ...
                // ถ้ามีคำไหนที่หาไม่เจอเลยแม้แต่อย่างเดียว ถือว่าเงื่อนไขโดยรวมไม่ผ่าน
                hasNoMatchOnSomeTerm = true;
                break; 
            }
        }

        // ถ้าพิมพ์หลายคำ แล้วมีคำแปลกปลอมที่หาไม่เจอเลย บังคับให้ตารางว่าง
...
window.openEditVisitView = async function(visitId) {
  ...
  // 🌟 ดึงภาษาปัจจุบันของระบบมาใช้งานกับปุ่มด้านล่าง
...
window.initVisitPage = async function(forceReload) {
    ...
    // 🌟 สาดหน้า Loading เฉพาะตอนที่ "ต้องไปดึงฐานข้อมูลใหม่" เท่านั้น (รองรับ 2 ภาษา)
