...
// คิวเก็บ Log e-Detailing ชั่วคราว (ยิงลง DB เมื่อกด Save เท่านั้น)
...
// ==========================================
// 📊 6. VIEW & UI RENDERERS (ตาราง และ ปฏิทิน)
// ==========================================
...
window.renderVisitTable = function() {
  ...
  paginatedData.forEach(function(v) {
    ...
    var docNameShow = (typeof window.getDoctorNameByLang === 'function') ? window.getDoctorNameByLang(docObj, v.Doc_ID) : '-';
    ...
    var hospNameShow = (docObj && typeof window.getHospitalNameFromDocOrVisit === 'function') ? window.getHospitalNameFromDocOrVisit(docObj, v) : '-';
    ...
    if (v.CheckIn_Lat && v.CheckIn_Long) {
      ...
      if (hospLat && hospLng) {
        ...
        if (distKm !== null && distKm <= 0.5) {
          distanceBadge = ' <a href="' + googleMapUrl + '" target="_blank" class="text-success ms-1" title="Check-in verified (<500m)">' +
                            '<i class="fa-solid fa-circle-check"></i>' +
                          '</a>';
        } else {
          var distShow = distKm < 1 ? Math.round(distKm * 1000) + 'm' : distKm.toFixed(1) + 'km';
          distanceBadge = ' <a href="' + googleMapUrl + '" target="_blank" class="text-danger ms-1" title="Off-site: ' + distShow + '">' +
                            '<i class="fa-solid fa-location-dot"></i>' +
                          '</a>';
        }
      } else {
        distanceBadge = ' <a href="' + googleMapUrl + '" target="_blank" class="text-secondary opacity-75 ms-1" title="Open Google Maps">' +
                          '<i class="fa-solid fa-location-dot"></i>' +
                        '</a>';
      }
    }
    ...
    htmlBuffer += '<tr>' +
      '<td class="text-center fw-bold"><a href="#" class="table-visit-link" onclick="if(typeof window.openEditVisitView === \'function\') window.openEditVisitView(\'' + v.Visit_ID + '\'); return false;">' + dateShow + '</a></td>' +
  ...
  if (typeof window.renderPaginationControls === 'function') window.renderPaginationControls(totalPages);
};
...
window.loadVisits = async function(forceReload) {
  ...
  if (forceReload || !window.VisitManagerCache.isLoaded) {
      ...
      try {
        var promises = [
          (typeof window.fetchAllRecords === 'function' ? window.fetchAllRecords('Visit_Logs') : []),
          (typeof window.fetchAllRecords === 'function' ? window.fetchAllRecords('Visit_Products') : []),
...
window.filterVisits = function(resetPage) {
  ...
  var filtered = window.globalVisits.filter(function(v) {
    ...
    if (window.myIsGlobalViewer) { 
        ...
        var rawWho = String(v.Whoupdated || v.whoupdated || '').toLowerCase().trim(); 
        var rawRepId = String(v.Rep_ID || v.rep_id || '').trim();

        if (window.myAllowedRepIds.indexOf(rawRepId) !== -1) {
            hasSecurityAccess = true;
        } else if (rawWho !== '' && window.myAllowedEmails.indexOf(rawWho) !== -1) {
            hasSecurityAccess = true;
        }
    }
    ...
    if (searchKeywords.length > 0) {
      ...
      var hospName = (typeof window.getHospitalNameFromDocOrVisit === 'function' && docObj) ? window.getHospitalNameFromDocOrVisit(docObj, v).toLowerCase() : '';
      ...
      var purposeShow = (typeof window.getPurposeText === 'function') ? window.getPurposeText(v.Purpose_ID).toLowerCase() : '';
  ...
  filtered.sort(function(a, b) {
    ...
    if (window.currentSortCol === 'date') { valA = a.Visit_Date || ''; valB = b.Visit_Date || ''; } 
      ...
      var docA = window._docIndex[String(a.Doc_ID || a.doc_id || a.id || '').trim().toLowerCase()]; valA = (typeof window.getDoctorNameByLang === 'function') ? window.getDoctorNameByLang(docA, a.Doc_ID).toLowerCase() : '';
      var docB = window._docIndex[String(b.Doc_ID || b.doc_id || b.id || '').trim().toLowerCase()]; valB = (typeof window.getDoctorNameByLang === 'function') ? window.getDoctorNameByLang(docB, b.Doc_ID).toLowerCase() : '';
    } 
    else if (window.currentSortCol === 'hospital') {
      var docA = window._docIndex[String(a.Doc_ID || a.doc_id || a.id || '').trim().toLowerCase()]; valA = (typeof window.getHospitalNameFromDocOrVisit === 'function' && docA) ? window.getHospitalNameFromDocOrVisit(docA, a).toLowerCase() : '';
      var docB = window._docIndex[String(b.Doc_ID || b.doc_id || b.id || '').trim().toLowerCase()]; valB = (typeof window.getHospitalNameFromDocOrVisit === 'function' && docB) ? window.getHospitalNameFromDocOrVisit(docB, b).toLowerCase() : '';
    } 
      ...
      valA = (typeof window.getPurposeText === 'function') ? window.getPurposeText(a.Purpose_ID).toLowerCase() : ''; 
      valB = (typeof window.getPurposeText === 'function') ? window.getPurposeText(b.Purpose_ID).toLowerCase() : '';
    } 
  ...
  if (typeof window.updateStatCards === 'function') window.updateStatCards(window.globalFilteredVisits);
  if (typeof window.renderVisitTable === 'function') window.renderVisitTable();
    ...
    try {
      ...
      if (Array.isArray(rawArr)) {
        window.currentAttachments = rawArr.map(function(item) {
          ...
          return {
            name: (typeof item === 'object' && item.name) ? item.name : fName,
            url: fileUrl,
            fileName: fName
          };
        });
...
window.openAddVisitView = async function(presetDate) {
  ...
  window.setFormComponentsReadOnly(false);
  window.savedSignatureData = null;
  window.currentAttachments = [];
  window.newlyUploadedFiles = [];
  window.pendingDeleteFiles = [];
...
window.handleSaveVisit = async function(e) {
  ...
  var sigData = window.getSignatureDataUrl();
  ...
  try {
    ...
    if (window.pendingDetailingLogs && window.pendingDetailingLogs.length > 0) {
      for (var dIdx = 0; dIdx < window.pendingDetailingLogs.length; dIdx++) {
        ...
        var logPayload = {
          ...
          Media_ID: itemLog.Media_ID,
          Duration_Seconds: itemLog.Duration_Seconds,
          Whenopend: itemLog.Whenopend,
          Whoupdated: itemLog.Whoupdated
        };
        ...
        try {
          ...
          if (!logRes.error && logRes.data && logRes.data.length > 0) {
            ...
            if (itemLog.Pages && itemLog.Pages.length > 0) {
              var pagePayloads = itemLog.Pages.map(function(pLog) {
                return {
                  Log_ID: createdLogId,
                  Page_Number: pLog.Page_Number,
                  Duration_Seconds: pLog.Duration_Second,
                  Whenopend: pLog.Whenopend,
                  Whoupdated: itemLog.Whoupdated
                };
              });
          ...
          console.error("Error saving detailing log:", detErr);
        }
      }
    }
    window.pendingDetailingLogs = [];
    ...
    if (window.pendingDeleteFiles && window.pendingDeleteFiles.length > 0) {
      ...
      if (sbClient && sbClient.storage) {
        try {
          var { data, error } = await sbClient.storage.from('visit-attachments').remove(window.pendingDeleteFiles);
          if (error) console.error("Storage delete error on save:", error.message);
          else console.log("Successfully deleted pending files from Storage:", data);
        } catch (err) {
          console.error("Error deleting pending files:", err);
        }
      }
    }
    window.newlyUploadedFiles = [];
...
window.cancelVisitForm = async function() {
  ...
  if (window.newlyUploadedFiles && window.newlyUploadedFiles.length > 0) {
    ...
    if (sbClient && sbClient.storage) {
      try {
        var { data, error } = await sbClient.storage.from('visit-attachments').remove(window.newlyUploadedFiles);
        if (error) console.error("Cancel cleanup error:", error.message);
        else console.log("Cleaned up new files on cancel:", window.newlyUploadedFiles);
      } catch(err) {
        console.error("Error during cancel cleanup:", err);
      }
    }
  }
...
window.toggleTextDictation = function(targetInputId, btnId) {
  ...
  if (window.textRecognition && window.currentDictTargetId === targetInputId) {
    if (typeof window.stopTextDictation === 'function') window.stopTextDictation();
    return;
  }
...
window.toggleSpeechSearch = function(inputId, btnId, iconId) {
  ...
  window.searchRecognition.onresult = function(event) {
    ...
    if (inputEl && spokenText) {
        ...
        if (typeof window.debouncedFilterVisits === 'function') window.debouncedFilterVisits();
    }
    if (typeof window.stopSearchDictation === 'function') window.stopSearchDictation();
  };
...
// ==========================================
// 📏 คำนวณระยะห่างระหว่าง 2 พิกัด (หน่วย: กิโลเมตร)
// ==========================================
...
window.handleFileUpload = async function(event) {
  ...
  else if (window.dbClient && window.dbClient.storage) sbClient = window.dbClient;

  if (!sbClient) {
    if (window.showToast) window.showToast("ไม่พบการเชื่อมต่อ Supabase Storage", "error");
    return;
  }
...
window.saveSignatureFromModal = function() {
  ...
  if (mCanvas.toDataURL() === blank.toDataURL()) {
    ...
    window.savedSignatureData = mCanvas.toDataURL('image/png');
  }
...
window.setFormComponentsReadOnly = function(isReadOnly) {
  ...
  if (fileBtnLabel) {
    if (isReadOnly) {
      fileBtnLabel.classList.add('disabled', 'pe-none', 'opacity-50');
    } else {
      fileBtnLabel.classList.remove('disabled', 'pe-none', 'opacity-50');
    }
  }
